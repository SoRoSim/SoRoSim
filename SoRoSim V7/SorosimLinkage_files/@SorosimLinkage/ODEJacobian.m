function dxd_dx = ODEJacobian(Linkage,t,x,dynamicAction,GUI_actionInput)
    ...
    if Linkage.nCLj>0 %if there are closed chain joints
        ...
        if Linkage.Actuated
            ...
            dFD_dq(Linkage.ActuationPrecompute.index_q_u,:) = dy_dq(:,1:ndof-n_k);
            dFD_dq(:,Linkage.ActuationPrecompute.index_q_k) = zeros(ndof,n_k);
            dFD_dqd(Linkage.ActuationPrecompute.index_q_u,:) = dy_dqd(:,1:ndof-n_k);
            dFD_dqd(:,Linkage.ActuationPrecompute.index_q_k) = zeros(ndof,n_k);
        else
