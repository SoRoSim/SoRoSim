...
function [y,C,B_action,action,z_star,J_f] = dynamicsSolver_multibody(Linkage,t,qqd,action) %x is [qdd_u (unknown); u_u; lambda], action is [u_k,qdd_k]
...
for i=1:N
    ...
    g((i_sig1-1)*4+1:i_sig1*4,:)  = g_here;
    J((i_sig1-1)*6+1:i_sig1*6,:)  = J_here;
    Jd((i_sig1-1)*6+1:i_sig1*6,:) = Jd_here;
    i_sig1                       = i_sig1+1;
    ...
    if Linkage.VLinks(Linkage.LinkIndex(i)).linktype=='r'
        ...
        if Linkage.UnderWater
            M_here = M_here+Linkage.M_added((i_sig1-1)*6+1:i_sig1*6,:); %Including Added Mass
            DL_here = Linkage.DL((i_sig1-1)*6+1:i_sig1*6,:);
    ...
    for j=1:Linkage.VLinks(Linkage.LinkIndex(i)).npie-1 %will run only if soft link
        ...
        if Ws(ii)>0
            ...
            if Linkage.UnderWater
                M_here  = M_here+Ws(ii)*Linkage.M_added((i_sig1-1)*6+1:i_sig1*6,:); %Including Added Mass
                DL_here = Ws(ii)*Linkage.DL((i_sig1-1)*6+1:i_sig1*6,:);
                v_here = norm(eta_here(4:6)); %linear speed
                F = F-J_here'*DL_here*v_here*eta_here; %drag lift force
            end
...
for icp = 1:Linkage.ncp
    ...
    if out.alpha < 1
        ...
        Fc1 = [dinamico_tilde(out.x)*fc; fc];
        ...
        Fc2 = - dinamico_coAdjoint(ginv(g12)) * Fc1;
...
for ip=1:Linkage.np
    ...
    J_here = J((i_sig1-1)*6+1:i_sig1*6,:);
...
if Linkage.CEF
    ...
    for i_sig1=1:Linkage.nsig
        F = F+J((i_sig1-1)*6+1:i_sig1*6,:)'*Fext;
    end
end
...
if Linkage.Actuated
    ...
    for ii=na_j1+1:N_jact %Links with actuated multi DOF joints
        ...
        if Linkage.VLinks(Linkage.LinkIndex(i)).jointtype=='C' %cylindrical
            ...
            i_sig1 = Linkage.ActuationPrecompute.i_sig_act(ii); %at CM of rigid body or X=0 of first soft division
            ...
            AdgstepinvS_here = dinamico_Adjoint(gi)*J((i_sig1-1)*6+1:i_sig1*6,i_jactq(i_u:i_u+dof_here-1)); %at X=1 of the joint
            Phi_here = Linkage.CVRods{i}(1).Phi;
