function [dID_dq,dID_dqd,dtau_dq,dtau_dqd,de_dq,de_dqd,daction_dq,daction_dqd] = DAEi1Jacobians_qqd_multibody(Linkage,t,q,qd,qdd,u,lambda,z_star,J_f)
%only Jacobians
...
i_sig1 = 1;     %current computational point index
...
for i=1:N
    ...
    %0 of joint
    g((i_sig1-1)*4+1:4*i_sig1,:) = g_here;

    J((i_sig1-1)*6+1:6*i_sig1,:)    = J_here;
    Jd((i_sig1-1)*6+1:6*i_sig1,:)   = Jd_here;
    eta((i_sig1-1)*6+1:6*i_sig1,:)  = eta_here;
    etad((i_sig1-1)*6+1:6*i_sig1,:) = etad_here;

    L_B((i_sig1-1)*6+1:6*i_sig1,:) = L_Bhere;
    R_B((i_sig1-1)*6+1:6*i_sig1,:) = R_Bhere;
    Q_B((i_sig1-1)*6+1:6*i_sig1,:) = Q_Bhere;
    Y_B((i_sig1-1)*6+1:6*i_sig1,:) = R_Bhere+Jd_here;
...
for icp = 1:Linkage.ncp
    ...
    if Linkage.Pairs(icp).contact_active
        ...
        Fc1 = [dinamico_tilde(x)*fc; fc];
        
        %to compute dz_dq = -J_f\dF_dq
        y = R12'*(x-r12)/alpha;
        y_tilde = dinamico_tilde(y);
        [grad2, H2] = Linkage.Pairs(icp).body2.get_gradH(y);
        J12 = J2-dinamico_Adjoint(ginv(g12))*J1; %relative Jacobian of 2 wrt 1 in frame of 2. J2-(Ad_g12^-1)*J1, J2 = 0 (fixed)
        ...
        dFc1_dq = [- dinamico_tilde(fc) * dz_dq(1:3,:) + dinamico_tilde(x) * df_dq; df_dq];
        dID_dq = dID_dq-J1'*dFc1_dq; %external force hence negative

        % body Jacobian (frame 1)
        Fk((i_sig1-1)*6+1:i_sig1*6) = Fk((i_sig1-1)*6+1:i_sig1*6)+Fc1;

        C21 = dinamico_coAdjoint(ginv(g12));   % coAd(g21)
        Fc2 = - C21 * Fc1;
        dFc2_dq = - C21 * dFc1_dq + C21 * dinamico_coadjbar(Fc1) * dinamico_Adjoint(g12)*  J12;
        dID_dq = dID_dq-J2'*dFc2_dq; %external force hence negative
        Fk((i_sig2-1)*6+1:i_sig2*6) = Fk((i_sig2-1)*6+1:i_sig2*6)+Fc2;
    end
end
...
for ip=1:Linkage.np
    ...
    if ~Linkage.LocalWrench(ip) %if in the global frame
        ...
        dFp_dq = -dinamico_coadjbar(Fp_here)*I_theta*J((i_sig1-1)*6+1:i_sig1*6,:);
        dID_dq = dID_dq-J((i_sig1-1)*6+1:i_sig1*6,:)'*dFp_dq; %external force hence negative
    end

    Fk((i_sig1-1)*6+1:i_sig1*6) = Fk((i_sig1-1)*6+1:i_sig1*6)+Fp_here;
end
...
if Linkage.CEF
    ...
    Fk = Fk+Fext;
    for i_sig1=1:Linkage.nsig
        dID_dq = dID_dq-J((i_sig1-1)*6+1:i_sig1*6,:)'*dFext_dq((i_sig1-1)*6+1:i_sig1*6,:);
    end
end
...
for i=N:-1:1 %backwards
    ...
    if Linkage.VLinks(Linkage.LinkIndex(i)).linktype=='r'
        ...
        i_sig1 = i_sig1-1; %at i_sig+1 is called
        ...
        F_ap1 = -M_ap1*dinamico_Adjoint(ginv(g(i_sig1*4+1:4*(i_sig1+1),:)))*G... %Gravity (external hence -ve)
                -Fk(i_sig1*6+1:6*(i_sig1+1)); %External point forces

        if Linkage.UnderWater
            M_ap1 = M_ap1+Linkage.M_added(i_sig1*6+1:(i_sig1+1)*6,:); %Including Added Mass
            %Including Drag lift force
            DL_here = Linkage.DL(i_sig1*6+1:(i_sig1+1)*6,:); %drag lift matrix
            v_here_norm = norm(eta(i_sig1*6+4:6*(i_sig1+1)));
            if v_here_norm>0
                DL_here_star = DL_here*(1/v_here_norm*eta(i_sig1*6+1:6*(i_sig1+1))*eta(i_sig1*6+1:6*(i_sig1+1))'*I_v+v_here_norm*eye(6));
            else
                DL_here_star = zeros(6,6);
            end

            N_ap1 = dinamico_coadjbar(M_ap1*eta(i_sig1*6+1:6*(i_sig1+1)))+dinamico_coadj(eta(i_sig1*6+1:6*(i_sig1+1)))*M_ap1-M_ap1*dinamico_adj(eta(i_sig1*6+1:6*(i_sig1+1)))+DL_here_star;
            F_ap1 = F_ap1+M_ap1*etad(i_sig1*6+1:6*(i_sig1+1))+dinamico_coadj(eta(i_sig1*6+1:6*(i_sig1+1)))*M_ap1*eta(i_sig1*6+1:6*(i_sig1+1))...%inertial and Coriolis; 
                   +DL_here*v_here_norm*eta(i_sig1*6+1:6*(i_sig1+1));%Drag lift
        else
            N_ap1 = dinamico_coadjbar(M_ap1*eta(i_sig1*6+1:6*(i_sig1+1)))+dinamico_coadj(eta(i_sig1*6+1:6*(i_sig1+1)))*M_ap1-M_ap1*dinamico_adj(eta(i_sig1*6+1:6*(i_sig1+1)));
            F_ap1 = F_ap1+M_ap1*etad(i_sig1*6+1:6*(i_sig1+1))+dinamico_coadj(eta(i_sig1*6+1:6*(i_sig1+1)))*M_ap1*eta(i_sig1*6+1:6*(i_sig1+1));%inertial and Coriolis;
        end 
        ...
        for j=Linkage.VLinks(Linkage.LinkIndex(i)).npie-1:-1:1
            ...
            nip = Linkage.CVRods{i}(j+1).nip;
            i_sig1 = i_sig1-1; %i_sig+1 is called
            ...
            for ii=nip-1:-1:1
                ...
                if Linkage.UnderWater
                    M_ap1 = M_ap1+Ws(ii+1)*Linkage.M_added(i_sig1*6+1:(i_sig1+1)*6,:); %Including Added Mass
                    %Including Drag lift force
                    DL_here = Ws(ii+1)*Linkage.DL(i_sig1*6+1:(i_sig1+1)*6,:); %drag lift matrix
                    v_here_norm = norm(eta(i_sig1*6+4:6*(i_sig1+1)));
                    if v_here_norm>0
                        DL_here_star = DL_here*(1/v_here_norm*eta(i_sig1*6+1:6*(i_sig1+1))*eta(i_sig1*6+1:6*(i_sig1+1))'*I_v+v_here_norm*eye(6));
                    else
                ...
                dID_dq(dofs_here,:) = dID_dq(dofs_here,:)+dSTdq_FC_full+S((ij-1)*6+1:6*ij,dofs_here)'*(N_C*R_B((i_sig1-1)*6+1:6*i_sig1,:)+M_C*Q_B((i_sig1-1)*6+1:6*i_sig1,:)+U_S+P_S);
                dID_dqd(dofs_here,:) = dID_dqd(dofs_here,:)+S((ij-1)*6+1:6*ij,dofs_here)'*(N_C*J((i_sig1-1)*6+1:6*i_sig1,:)+M_C*Y_B((i_sig1-1)*6+1:6*i_sig1,:)+V_S);
            ...
            if Linkage.UnderWater
                M_ap1 = M_ap1+Ws(1)*Linkage.M_added(i_sig1*6+1:(i_sig1+1)*6,:); %Including Added Mass
                %Including Drag lift force
                DL_here = Ws(1)*Linkage.DL(i_sig1*6+1:(i_sig1+1)*6,:); %drag lift matrix
                v_here_norm = norm(eta(i_sig1*6+4:6*(i_sig1+1)));
                if v_here_norm>0
                    DL_here_star = DL_here*(1/v_here_norm*eta(i_sig1*6+1:6*(i_sig1+1))*eta(i_sig1*6+1:6*(i_sig1+1))'*I_v+v_here_norm*eye(6));
                else
